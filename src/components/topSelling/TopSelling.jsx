...
function TopSelling() {
  ...
  return (
    <section className='mt-[110px] font-poppins flex flex-col space-y-9'>
        ...
        <div className='w-[95vw] h-[]  lg:w-[1000px] mx-auto  flex
        flex-wrap gap-y-3 justify-between   '>   
           {
            catergoryTop.map((item, index) => {
                return (
                    <div key={index} onClick={()=>topHandler(item.id)}
                     className={`w-[45%] h-[175px] md:w-[150px]  lg:w-[200px] lg:h-[200px] cursor-pointer rounded-3xl
                         flex flex-col  justify-center items-center gap-y-1
                    bg-[#fffff] shadow-[0_0px_4px_rgba(0,0,0,0.3)]
                   ${selectCategoryId===item.id ?
                   "bg-[#EB5757 bg-black ":""} `}>
                    <div className={`w-[110px] h-[110px] bg-[#C1F1C6] rounded-full
                    flex  justify-center items-center hover:scale-105 border hover:border-green-500
                    hover:brightness-90
                     ${selectCategoryId===item.id ?
                   " bg-slate-50 ":""} `}>
                        <img src={item.image} alt="" className='w-[70px] h-[70px] rounded-full
                        hover:scale-105' />
                         
                    </div>
                  
                    <p className={`text-[#1E1E1E] text-[1.2rem] font-semibold
                     ${selectCategoryId===item.id ?
                    ...
                    return(
                        ...
                        hover:scale-105 transition-shadow duration-500 '>
                            ...
                            <img src={item.image} alt="" className='w-[80%] border-black h-[195px]
                             ...
                             <p className='text-gray-500'>fresh food 100% guaranty</p>
